...
is full, data in the buffer is sent to the destination core by several packets. We could easily see that data will be
sent in \textit{bursts} of packets. This accumulation mechanism could help
increase the \textit{silent} intervals between packages and thus reducing the
number of time to turn-on/off links and saving more power as turning links
on/off consumes some amount of power.
...
5. Figure~\ref{fig:packettime} shows timestamp packets sent between node 4 and
5, we then take the difference of the timestamps of the packets to get the interval between
...
\begin{figure*}[ht!]
\centering
\subfigure[Package sending time]{
...
\subsection{Correlation of Package Send/Receive Times}
