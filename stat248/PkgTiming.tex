...
machine. For example, in Figure~\ref{fig:comm}, an SDF application can be partitioned into
...
first \textit{pushed} into a buffer as in Figure~\ref{fig:comm}. When the buffer
is full, data in the buffer is sent to the destination core in packets. As a
buffer size is several times larger than a packet size, so when a buffer is
sent, several continuous packets are sent and it looks like a \textit{burst} of
packets. This accumulation mechanism could help increase the \textit{silent}
intervals between Packets and thus reducing the number of time to turn-on/off
links and saving more power as turning links on/off consumes some amount of power.
...
\caption{Packet interval Autocorrelation}\label{fig:Cluster}
\end{figure}
...
5. Figure~\ref{fig:packettime} shows timestamps of packets sent between node 4
and 5, we then take the difference of the timestamps to get the intervals
between successive packets sent from cluster 4 to cluster 5 as in
Figure~\ref{fig:s45} as the project aims at predicting the intervals between packets. 
...
Autocovariance measures the linear dependence between two points in the same
series at different times.
\begin{equation}
\gamma(s,t)=cov(Y_s, Y_t)=E[(Y_s-\mu_s)(Y_t-\mu_t)]
\end{equation}
where $\mu_t=E[Y_t]$. Autocorrelation measures the predictability of the series
...
\caption{Residual series after removing seasonal}\label{s45res.pdf}
\end{figure}
